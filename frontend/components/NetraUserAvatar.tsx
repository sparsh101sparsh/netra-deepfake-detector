...
export const NetraUserAvatar: React.FC<NetraUserAvatarProps> = ({
  ...
  return (
    <div
      className={`relative rounded-full flex items-center justify-center overflow-hidden bg-gradient-to-br ${avatar.bgGradient} border ${avatar.borderColor} ${avatar.textColor} ${className}`}
      style={{
        width: `${size}px`,
        height: `${size}px`,
        boxShadow: showGlow ? `0 0 12px ${avatar.glowColor}` : "none",
        flexShrink: 0,
      }}
      title={avatar.name}
    >
      {avatar.imageUrl && !imgError ? (
        <img
          src={avatar.imageUrl}
          alt={avatar.name}
