...
def build_court_ready_forensic_pdf(
    ...
    story = [
        ...
        Spacer(1, 2),
        Paragraph("Certified under Section 65B Indian Evidence Act / Section 63 BSA 2023 &amp; Section 66D IT Act 2000", sub_s),
        Spacer(1, 6),
    ...
    for snap in keyframe_snapshots:
        ...
        statute = snap.get("statutory_act", "Section 65B Indian Evidence Act & Section 66D IT Act 2000")
        ...
        right_details = [
            [Paragraph("Keyframe / Timestamp:", cell_bold), Paragraph(f"<b>Frame #{f_num} @ {ts}</b>", cell_norm)],
            [Paragraph("Neural Anomaly Index:", cell_bold), Paragraph(f"<font color='#dc2626'><b>{score:.1f}% (CRITICAL ANOMALY)</b></font>", cell_norm)],
            [Paragraph("Localized Anomaly Region:", cell_bold), Paragraph(f"<b>{region}</b>", cell_norm)],
            [Paragraph("Detector Subsystem:", cell_bold), Paragraph(subsys, cell_norm)],
            [Paragraph("Statutory Evidence Act:", cell_bold), Paragraph(statute, cell_norm)],
            [Paragraph("Forensic Annotation:", cell_bold), Paragraph("Amber border (#f59e0b) isolates localized boundary artifacts.", cell_norm)],
        ]
        t_right = Table(right_details, colWidths=[105, 175])
        ...
        side_by_side = Table([[img_flowable, t_right]], colWidths=[230, 290])
    ...
    legal_text = (
        ...
        "court-admissible electronic evidence pursuant to <b>Section 65B of the Indian Evidence Act 1872</b> / "
        "<b>Section 63 of the Bharatiya Sakshya Adhiniyam (BSA) 2023</b>. Detected synthetic impersonation "
...
class TestBenchmark20Videos:
    ...
    def test_02_individual_video_pipeline_and_artifacts(self, video_filename: str):
        ...
        assert f"Frame #{f_indices[0]}" in page_text
